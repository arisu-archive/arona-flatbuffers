...
func (t *MessagePopupExcelDto) MarshalModel(b *flatbuffers.Builder) flatbuffers.UOffsetT {
	...
	for i := range len(t.ButtonCommand) {
		b.PrependUOffsetT(fbsutils.Convert(b.CreateString(t.ButtonCommand[len(t.ButtonCommand)-i-1]), t.FlatBuffer.TableKey))
	}
	...
	for i := range len(t.Button) {
		b.PrependInt32(fbsutils.Convert(int32(fbsutils.Convert(t.Button[len(t.Button)-i-1], t.FlatBuffer.TableKey)), t.FlatBuffer.TableKey))
	}
	...
	for i := range len(t.ButtonParameter) {
		b.PrependUOffsetT(fbsutils.Convert(b.CreateString(t.ButtonParameter[len(t.ButtonParameter)-i-1]), t.FlatBuffer.TableKey))
	}
	...
	MessagePopupExcelAddDisplayXButton(b, fbsutils.Convert(t.DisplayXButton, t.FlatBuffer.TableKey))
	MessagePopupExcelAddImage(b, fbsutils.Convert(b.CreateString(t.Image), t.FlatBuffer.TableKey))
